...
def prepare_runtime(state: TrainState, services: Services) -> dict[str, Any]:
    ...
    services.artifacts.write_json(
        ...
        {"dataset_uri": state["dataset_uri"], "dataset_version": state["dataset_version"]},
    )
    dataset = Path(state["dataset_uri"])
    return {
        "updated_at": now_iso(),
        "workflow_status": "optimizing",
        "train_dataset_uri": str(dataset),
        "validation_dataset_uri": str(dataset),
        "test_dataset_uri": str(dataset),
